...
const inputCls = 'w-full px-2 py-1 text-xs bg-white border border-slate-200 rounded-lg outline-none focus:ring-1 focus:ring-indigo-400 placeholder:text-slate-300';
const selectCls = 'w-full px-2 py-1 text-xs bg-white border border-slate-200 rounded-lg outline-none focus:ring-1 focus:ring-indigo-400';
...
export function TransactionsView() {
  ...
  const set = (k: keyof ColFilters) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
  ...
  const activeFilters = useMemo(
    () => Object.entries(f).filter(([, v]) => v !== '').length,
    [f],
  );
  ...
  const display = useMemo(() => {
    ...
    if (f.description)
      list = list.filter(tx => (tx.description ?? '').toLowerCase().includes(f.description.toLowerCase()));
    if (f.type)
      list = list.filter(tx => tx.type === f.type);
    if (f.category)
      list = list.filter(tx => tx.category === f.category);
    if (f.amountMin !== '')
      list = list.filter(tx => (tx.amount ?? 0) >= parseFloat(f.amountMin));
    if (f.amountMax !== '')
      list = list.filter(tx => (tx.amount ?? 0) <= parseFloat(f.amountMax));
    if (f.dateFrom)
      list = list.filter(tx => !!tx.date && tx.date >= f.dateFrom);
    if (f.dateTo)
      list = list.filter(tx => !!tx.date && tx.date <= f.dateTo);
  ...
  useEffect(() => {
    ...
    const visibleIds = display.map(t => t.id);
    const checkedCount = visibleIds.filter(id => selected.has(id)).length;
    selectAllRef.current.checked = checkedCount > 0 && checkedCount === visibleIds.length;
    selectAllRef.current.indeterminate = checkedCount > 0 && checkedCount < visibleIds.length;
  }, [selected, display]);
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-lg mx-4">
            ...
            {missingFields(editForm).length > 0 && (
              <div className="mb-5 flex items-start gap-2 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-700 text-sm">
                ...
                <span>Missing: <strong>{missingFields(editForm).join(', ')}</strong> — fill in all fields to mark as complete.</span>
              </div>
            )}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                    ...
                    {CATEGORIES[editForm.type].map(cat => <option key={cat} value={cat}>{cat}</option>)}
                  </select>
                </div>
              </div>
            </div>
          ...
          {selectedCount > 0 ? (
            <>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-rose-50 text-rose-600 border border-rose-200 hover:bg-rose-100 transition-all disabled:opacity-50"
              >
                <Trash2 size={13} />{bulkDeleting ? 'Deleting…' : `Delete ${selectedCount}`}
              </button>
              <button
                onClick={() => setSelected(new Set())}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-slate-100 text-slate-500 border border-slate-200 hover:bg-slate-200 transition-all"
              >
            ...
            <thead>
              ...
              <tr className="border-b border-slate-100 bg-slate-50/60">
                ...
                <td className="px-3 py-2">
                  <input type="text" placeholder="Filter description…" value={f.description}
              ...
              {display.map(tx => {
                ...
                return (
                  <tr key={tx.id}
                    ...
                    <td className="px-4 py-3.5">
                      {tx.type ? (
                        <span className={`text-[10px] font-black px-2 py-1 rounded-md whitespace-nowrap ${
                          ...
                          tx.type === 'cost'       ? 'bg-amber-100  text-amber-700'   :
                    ...
                    <td className="px-4 py-3.5 text-right text-sm text-slate-400 tabular-nums">
                      {tx.date ?? <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-4 py-3.5">
